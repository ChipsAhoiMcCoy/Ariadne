...
- `Player.GetItem(int, Item, GetItemSettings)` is the single funnel every item takes into the inventory, but it has no hook. The supported observation points are `ModPlayer.OnPickup(Item)` for world items and `ModPlayer.ModifyCaughtFish(Item)` for fishing, which reaches `GetItem` directly and never creates a world item. `Player.GrabItems` runs `CombinedHooks.OnPickup` on hitbox contact without any space test, so a full inventory still fires the hook; gate on public `Player.ItemSpace(Item).CanTakeItem`. `Player.PickupItem` consumes `ItemID.Sets.IsAPickup` and `ItemID.Sets.NebulaPickup` types as effects before `GetItem`, so those never produce a slot or vanilla pickup text. `PopupText.NewText(PopupTextContext, Item, int, ...)` is the vanilla pickup notification and merges same-name entries and coin values; it is suppressed entirely by `Main.showItemText` and by `GetItemSettings.NoText`, making the `Main.popupText` array unsuitable as an accessibility source. `Main.ValueToCoins(long)` renders a merged copper value as localized coin names.
