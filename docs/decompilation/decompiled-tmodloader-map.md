...
- Keep hostile emitter qualification and distance relative to the full `Main.Camera.ScaledPosition`/`ScaledSize` rectangle. For horizontal audio, normalize each side from the player's X position to its corresponding viewport edge, then apply a monotonic center-expansion curve that preserves player center `0` and screen edges `-1`/`+1`. Continue mapping viewport top/bottom to Y `-1`/`+1`. This remains zoom- and resolution-aware while making short central pulses easier to lateralize.
- A packaged hostile pulse is read with the owning `Mod.GetFileBytes("Assets/Audio/HostileMobPulse.wav")`. Validate RIFF/WAVE chunk lengths and mono 16-bit PCM format before decoding; keep this asset-backed stream separate from procedural accessibility streams so a content or device failure has a narrow failure boundary.
