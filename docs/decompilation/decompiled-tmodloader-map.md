...
| Tile semantics and object roots | `Terraria/Main.cs` (`IsTileSpelunkable`), `Terraria/Map/MapHelper.cs` (`CreateMapTile`, `TileToLookup`), `Terraria/Lang.cs` (`GetMapObjectName`), and `Terraria/ObjectData/TileObjectData.cs` (`GetTileData`, `TopLeft`) |
...
- Keep hostile emitter qualification and distance relative to the full `Main.Camera.ScaledPosition`/`ScaledSize` rectangle. For horizontal audio, normalize each side from the player's X position to its corresponding viewport edge, then apply a monotonic center-expansion curve that preserves player center `0` and screen edges `-1`/`+1`. Continue mapping viewport top/bottom to Y `-1`/`+1`. This remains zoom- and resolution-aware while making central vector movement and impacts easier to lateralize.
- Hostile awareness uses a dedicated procedural dynamic stereo stream. A filtered-noise vector starts at the player's normalized viewport position, follows the center-expanded horizontal path to a snapshot of the hostile target, and ends in a localized noise-and-sine impact. Apply the shared per-sample ILD/ITD transform directly to authored motion rather than smoothing it as an environmental emitter target. Keep this stream separate from wall tones so an audio failure has a narrow boundary.
