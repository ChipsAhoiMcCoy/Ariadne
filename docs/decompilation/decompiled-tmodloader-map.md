...
- Visible entities should be filtered by viewport intersection and at least one positive-brightness tile beneath the clipped hitbox. Multipart NPC segments share `NPC.realLife`; resolve the active root once and union only the visible qualifying segment bounds for the snapshot.
