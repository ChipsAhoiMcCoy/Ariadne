...
| Journey research and duplication | Purpose-built research actions and duplication list with an alternate actions pane | Researches eligible inventory items; Enter on a researched item duplicates its full stack to the cursor, while Tab exposes take-one, direct-to-inventory, and item-detail options. Journey powers retain their live stock screen with localized labels and native strip hierarchy through the adapter |
| Bestiary and emotes | Specialized profiles in the universal `UIState` adapter | Bestiary opens directly into one continuous semantic entry list without page buttons or an extra options level. Emotes expose every named emote without a selectable scrollbar. Both use Right to open/activate and Left to return |
