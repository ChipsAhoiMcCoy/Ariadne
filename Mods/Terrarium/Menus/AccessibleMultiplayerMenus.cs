...
internal sealed class AccessibleHostConfigurationMenuState : AccessibleMenuState
{
	...
	private void StartServer()
	{
		...
		catch (Exception exception)
		{
			...
			ModContent.GetInstance<TerrariumMod>().Logger.Error("Custom Host and Play launch failed.", exception);
		}
	}
}
...
internal sealed class AccessibleServerPasswordRequestState : UIState
{
	...
	private void Submit(string password)
	{
		...
		{
			TerrariumMod.ScreenReader.Output("The password could not be submitted because this tModLoader version changed its network API.");
			return;
		}
