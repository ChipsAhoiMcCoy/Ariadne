...
internal sealed class AccessibleTextInputState : UIState
{
	...
	public override void OnActivate()
	{
		...
		TerrariumMod.ScreenReader.Output($"{_prompt}. Edit field. {contents}. Press Enter to accept, Escape to cancel, or F1 for contextual help.");
	}
	...
	private void AnnounceEdit(string oldValue, string newValue)
	{
		if (_hideContents)
		{
			TerrariumMod.ScreenReader.Output($"{newValue.Length} characters.", interrupt: false);
			return;
		}

		if (newValue.Length == oldValue.Length + 1 && newValue.StartsWith(oldValue, StringComparison.Ordinal))
		{
			TerrariumMod.ScreenReader.Output(newValue[^1].ToString(), interrupt: false);
		}
		...
		{
			TerrariumMod.ScreenReader.Output(newValue, interrupt: false);
		}
	}
}
