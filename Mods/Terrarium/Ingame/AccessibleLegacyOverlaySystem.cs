...
internal sealed class AccessibleLegacyOverlaySystem : ModSystem
{
	...
	private void MonitorChat(KeyboardState keyboard)
	{
		...
		else if (!Main.drawingPlayerChat && _wasChatOpen)
		{
			...
			TerrariumMod.ScreenReader.Output("Chat closed.");
			return;
		}
		...
		{
			TerrariumMod.ScreenReader.Output(string.IsNullOrEmpty(value) ? "Empty" : value, interrupt: false);
		}
	}
	...
	private void MonitorDeathScreen()
	{
		...
		if (player.dead && !_wasDead)
		{
			...
			TerrariumMod.ScreenReader.Output($"You died.{coins} Respawning in {_lastRespawnSecond} seconds.");
			return;
		}
		if (!player.dead && _wasDead)
		{
			...
			TerrariumMod.ScreenReader.Output("Respawned.");
			return;
		}
		if (!player.dead)
		{
		...
		{
			TerrariumMod.ScreenReader.Output($"Respawning in {seconds} seconds.", interrupt: false);
		}
	}
