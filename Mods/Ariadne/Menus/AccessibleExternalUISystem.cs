...
/// that Ariadne has not replaced with a purpose-built accessible state.
/// </summary>
...
internal sealed class AccessibleExternalUISystem : ModSystem
{
	...
	private static bool IsAriadneState(UIState state)
	{
		return state.GetType().Namespace?.StartsWith("Ariadne", StringComparison.Ordinal) == true;
	}
}
...
internal sealed class AccessibleExternalUIController
{
	...
	private bool HandleInput(KeyboardState keyboard, long now)
	{
		...
		{
			AriadneMod.ScreenReader.Output(_controls[_selectedIndex].Details());
		}
	...
	private void SelectBestiaryEntry(UIBestiaryTest bestiary, BestiaryEntry entry, int entryIndex)
	{
		...
		AriadneMod.ScreenReader.Output(DescribeBestiaryEntry(entry).Details);
	}
	...
	private void NavigateByFirstLetter(char letter)
	{
		...
		if (index < 0)
		{
			AriadneMod.ScreenReader.Output($"No control starting with {char.ToUpperInvariant(letter)}.");
			return;
		}
		SetSelection(index);
	}
	...
	private static bool TryActivateEmote(UIElement element)
	{
		...
		AriadneMod.ScreenReader.Output($"{Lang.GetEmojiName(emoteIndex).Value} activated.");
		return true;
	}
	...
	private static void ActivateElement(UIElement element, bool secondary, string label)
	{
		...
		{
			AriadneMod.ScreenReader.Output($"{label} could not be activated: {exception.GetBaseException().Message}");
		}
		finally
		{
			if (secondary)
	...
	private void ReadHelp()
	{
		...
		AriadneMod.ScreenReader.Output(
			...
			"Text fields use the screen's native editor after activation. Escape uses the screen's normal back or cancel behavior. This semantic adapter is the fallback for stock and mod screens without a purpose-built Ariadne menu.");
	}
	...
	private void AnnounceChangedStatus(long now)
	{
		...
		_lastStatus = status;
		AriadneMod.ScreenReader.Output(status, interrupt: false);
	}
	...
	private void MonitorTextEditing()
	{
		...
		if (_lastEditValue is null)
		{
			...
			AriadneMod.ScreenReader.Output(hidden ? $"Edit field, {value.Length} characters." : $"Edit field. {value}");
			return;
		}
		...
		if (hidden)
		{
			AriadneMod.ScreenReader.Output($"{value.Length} characters.", interrupt: false);
		}
		...
		{
			AriadneMod.ScreenReader.Output(value, interrupt: false);
		}
	}
